...
from arachneserver.scrapy_utils import (create_crawler_object, start_logger,
...
class TestScrapyUtils(TestCase):
    ...
    def test_create_crawler_mock(self):
        if SCRAPY_VERSION <= (1, 0, 0):
            with patch('arachneserver.scrapy_utils.Crawler') as mock_crawler:
        ...
        if SCRAPY_VERSION <= (1, 0, 0):
            with patch('arachneserver.scrapy_utils.tlog') as tlog:
                with patch('arachneserver.scrapy_utils.load_object') as load_object:
                    with patch('arachneserver.scrapy_utils.Crawler') as _:
                        spider_loc = 'ABC.ABC'
